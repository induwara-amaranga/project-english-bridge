...
export function ProgressPage() {
  ...
  const stageRows = curriculum.stages.map((stage, i) => {
    ...
    if (status === 'completed') { label = isSi ? 'සම්පූර්ණයි' : '100% complete'; labelColor = '#3E8E5B'; barColor = 'var(--c-success)'; badgeBg = 'var(--c-success)'; }
    else if (status === 'skipped') { label = isSi ? 'ස්ථානගත කර ඇත' : 'Placed out — not reviewed'; labelColor = '#B8860B'; barColor = '#F5C842'; badgeBg = '#F5C842'; }
    else if (status === 'current') { label = isSi ? `${pct}% සම්පූර්ණයි` : `${pct}% complete`; labelColor = 'var(--c-primary)'; barColor = 'var(--c-primary)'; badgeBg = 'var(--c-primary)'; }
    else { label = isSi ? 'අගුළු දමා ඇත' : 'Locked'; labelColor = 'var(--c-ink-disabled)'; barColor = 'var(--c-primary-tint)'; badgeBg = '#D8D2F0'; }
    ...
    <div className="app-shell app-shell--nav-pad" style={{ background: 'var(--c-bg)', color: 'var(--c-ink)' }}>
      <div style={{ background: 'var(--c-primary)', padding: '20px 24px' }}>
        <div style={{ maxWidth: 640, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ fontWeight: 800, fontSize: 18, color: 'white', fontFamily: 'var(--font-display)' }}>{isSi ? 'ඔබේ ප්‍රගතිය' : 'Your Progress'}</div>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '28px 24px 0', display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div className="card">
            ...
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 24 }}>{isSi ? `මුළු XP ${progress.xp.toLocaleString()}` : `${progress.xp.toLocaleString()} XP total`}</div>
            <div style={{ fontSize: 13, color: 'var(--c-ink-soft)', fontWeight: 600, marginTop: 4 }}>{isSi ? `අදියර ${completed + 1}ක් හරහා උපයා ඇත` : `Earned across ${completed + 1} stages`}</div>
          </div>
          <div className="card">
            <div style={{ marginBottom: 10 }}><img src="/assets/icons/streak.svg" alt="" width={30} height={30} /></div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 24 }}>{isSi ? `දින ${progress.streakDays} අඛණ්ඩව` : `${progress.streakDays}-day streak`}</div>
            ...
            <div style={{ fontSize: 13, color: 'var(--c-ink-soft)', fontWeight: 600, marginTop: 4 }}>{isSi ? 'දිගටම කරගෙන යන්න — හෙට නැවත එන්න' : 'Keep it going — come back tomorrow'}</div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div className="card">
            ...
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 24 }}>{isSi ? `කාසි ${progress.coins.toLocaleString()}` : `${progress.coins.toLocaleString()} coins`}</div>
            <div style={{ fontSize: 13, color: 'var(--c-ink-soft)', fontWeight: 600, marginTop: 4 }}>{isSi ? 'අඛණ්ඩතා ශීතකරණ හෝ ප්‍රශ්න නැවත උත්සාහ කිරීම් සඳහා වියදම් කරන්න' : 'Spend on streak freezes or question retries'}</div>
          </div>
          <div className="card">
            ...
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 24 }}>{isSi ? `මට්ටම ${progress.level}` : `Level ${progress.level}`}</div>
            ...
            <div style={{ fontSize: 13, color: 'var(--c-ink-soft)', fontWeight: 600 }}>{isSi ? `ඊළඟ මට්ටමට XP ${progress.xpIntoLevel} / ${progress.xpForNextLevel}` : `${progress.xpIntoLevel} / ${progress.xpForNextLevel} XP to next level`}</div>
          </div>
        </div>
        ...
        <div className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, flexWrap: 'wrap' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 15 }}>
              ...
              {isSi ? `අඛණ්ඩතා ශීතකරණය ×${progress.streakFreezes}` : `Streak freeze ×${progress.streakFreezes}`}
            </div>
            <div style={{ fontSize: 13, color: 'var(--c-ink-soft)', fontWeight: 600, marginTop: 2 }}>{isSi ? 'ඔබ දිනක් මග හැරුණු විට ඔබේ අඛණ්ඩතාව ආරක්ෂා කරයි' : 'Protects your streak the next time you miss a day'}</div>
          ...
          >
            {buying ? (isSi ? 'මිලදී ගනිමින්…' : 'Buying…') : (isSi ? `කාසි ${STREAK_FREEZE_COST_COINS}කට මිලදී ගන්න` : `Buy for ${STREAK_FREEZE_COST_COINS} coins`)}
          </button>
        </div>

        <div className="card">
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 15, marginBottom: 6 }}>{isSi ? `අදියර ${curriculum.stages.length}න් ${completed}ක් සම්පූර්ණයි — පාඨමාලාවෙන් ${overallPct}%` : `${completed} of ${curriculum.stages.length} stages complete — ${overallPct}% of the course`}</div>
          ...
          <div style={{ fontSize: 13, color: 'var(--c-ink-soft)', fontWeight: 600 }}>{isSi ? `ඔබේ සහතිකය ලැබීමට තව අදියර ${stagesToGo}ක් ඉතිරිය` : `${stagesToGo} stages to go until your certificate`}</div>
        </div>

        <div className="card">
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 13, color: 'var(--c-primary)', marginBottom: 16 }}>{isSi ? 'අදියර විස්තරය' : 'STAGE BREAKDOWN'}</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {stageRows.map((row) => (
              <div key={row.stage.id} style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 5 }}>{isSi ? (row.stage.title.si || row.stage.title.en) : row.stage.title.en}</div>
