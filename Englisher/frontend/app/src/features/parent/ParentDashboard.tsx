...
export function ParentDashboard() {
  ...
  return (
    <div style={{ fontFamily: 'var(--font-body)', background: 'var(--c-bg)', color: 'var(--c-ink)', minHeight: '100vh' }}>
      ...
      {loading && (
        <div style={{ maxWidth: 640, margin: '0 auto', padding: '48px 24px', textAlign: 'center', color: 'var(--c-ink-soft)', fontWeight: 600 }}>{isSi ? 'පූරණය වෙමින්…' : 'Loading…'}</div>
      )}
      ...
      {!loading && (notLinked || !data) && (
        <div style={{ maxWidth: 640, margin: '0 auto', padding: '48px 24px 64px' }}>
          <div className="card" style={{ padding: 32, textAlign: 'center' }}>
            ...
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 20, marginBottom: 8 }}>{isSi ? NOT_LINKED_COPY.si.title : NOT_LINKED_COPY.en.title}</div>
            <div style={{ fontSize: 14, lineHeight: 1.65, color: 'var(--c-ink-soft)', fontWeight: 500, maxWidth: 420, margin: '0 auto' }}>{isSi ? NOT_LINKED_COPY.si.body : NOT_LINKED_COPY.en.body}</div>
          </div>
        </div>
      )}
        ...
        <div style={{ maxWidth: 640, margin: '0 auto', padding: '28px 24px 64px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div className="card">
              ...
              <div style={{ fontSize: 13, color: 'var(--c-ink-soft)', fontWeight: 600, marginTop: 4 }}>{isSi ? 'මෙතෙක් උපයා ඇත' : 'Earned so far'}</div>
            </div>
            <div className="card">
              <img src="/assets/icons/streak.svg" alt="" width={30} height={30} style={{ display: 'block', marginBottom: 10 }} />
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 21 }}>{isSi ? `දින ${data.progress.streakDays} අඛණ්ඩව` : `${data.progress.streakDays}-day streak`}</div>
              <div style={{ fontSize: 13, color: 'var(--c-ink-soft)', fontWeight: 600, marginTop: 4 }}>{isSi ? 'දිගටම කරගෙන යන්න' : 'Keep it going'}</div>
            </div>
          </div>

          <div className="card">
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 13, color: 'var(--c-primary)', marginBottom: 12 }}>{isSi ? 'අදියර ප්‍රගතිය' : 'STAGE PROGRESS'}</div>
            <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 6 }}>
              {isSi
                ? `දැනට සිටින්නේ: අදියර ${data.completedStages + 1}${data.stageName ? ` — ${data.stageName}` : ''}`
                : `Currently on: Stage ${data.completedStages + 1}${data.stageName ? ` — ${data.stageName}` : ''}`}
            </div>
            <div style={{ fontSize: 14, color: 'var(--c-ink-soft)', fontWeight: 600, marginBottom: 14 }}>{isSi ? `අදියර ${data.totalStages}න් ${data.completedStages}ක් සම්පූර්ණයි` : `${data.completedStages} of ${data.totalStages} stages complete`}</div>
          ...
          {data.recentWriting.length > 0 && (
            <div className="card">
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 13, color: 'var(--c-primary)', marginBottom: 12 }}>{isSi ? 'මෑත ලේඛන' : 'RECENT WRITING'}</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {data.recentWriting.map((w, i) => (
                  <div key={i} style={{ borderBottom: i === data.recentWriting.length - 1 ? 'none' : '1px solid var(--c-line)', paddingBottom: i === data.recentWriting.length - 1 ? 0 : 12 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--c-ink-soft)', marginBottom: 4 }}>{w.lessonId} · {isSi ? (w.status === 'submitted' ? 'ඉදිරිපත් කළා' : 'කටුව') : (w.status === 'submitted' ? 'Submitted' : 'Draft')}</div>
                    <div style={{ fontSize: 13.5, color: 'var(--c-ink-2)', lineHeight: 1.5 }}>{w.excerpt || (isSi ? '(තවම කිසිවක් ලියා නැත)' : '(nothing written yet)')}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ background: 'var(--c-primary-tint)', borderRadius: 20, padding: 22 }}>
            <p style={{ margin: 0, fontSize: 15, lineHeight: 1.6, fontWeight: 700, color: '#3D3468' }}>{isSi ? 'උදව් කිරීමට හොඳම ක්‍රමය: ඔවුන් අද ලකුණු ගත්තේ කොපමණද යන්න නොව, ඉගෙනගත්තේ කුමක්ද කියා අහන්න.' : 'The best way to help: ask what they learned today, not how much they scored.'}</p>
          </div>
